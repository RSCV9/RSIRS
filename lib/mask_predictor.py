...
# from arc.adaptive_rotated_conv import batch_rotate_multiweight_modified
from arc.adaptive_rotated_conv import batch_rotate_multiweight
import torch
from torch import nn
from torch.nn import functional as F
from collections import OrderedDict
...
class SimpleDecoding_modified(nn.Module):
    def __init__(self, c4_dims, factor=2):
        super(SimpleDecoding_modified, self).__init__()
        ...
        self.conv1_4 = nn.Conv2d(c4_size+c3_size, hidden_size, 3, padding=1, bias=False)
        self.routing_function1 = RountingFunction(in_channels=hidden_size, kernel_number=1)
        self.conv2_4 = AdaptiveRotatedConv2d(in_channels=hidden_size, out_channels=hidden_size,
                                                     kernel_size=3, padding=1, rounting_func=self.routing_function1, bias=False, kernel_number=1)
        ...
        self.conv1_3 = nn.Conv2d(hidden_size + c2_size, hidden_size, 3, padding=1, bias=False)
        self.routing_function2 = RountingFunction(in_channels=hidden_size, kernel_number=1)
        self.conv2_3 = AdaptiveRotatedConv2d(in_channels=hidden_size, out_channels=hidden_size,
                                             kernel_size=3, padding=1, rounting_func=self.routing_function2, bias=False, kernel_number=1)
        self.bn1_3 = nn.BatchNorm2d(hidden_size)
        self.relu1_3 = nn.ReLU()
        self.bn2_3 = nn.BatchNorm2d(hidden_size)
        self.relu2_3 = nn.ReLU()
        ...
        self.conv1_2 = nn.Conv2d(hidden_size + c1_size, hidden_size, 3, padding=1, bias=False)
        self.routing_function3 = RountingFunction(in_channels=hidden_size, kernel_number=1)
        self.conv2_2 = AdaptiveRotatedConv2d(in_channels=hidden_size, out_channels=hidden_size,
                                             kernel_size=3, padding=1, rounting_func=self.routing_function3, bias=False, kernel_number=1)
        self.bn1_2 = nn.BatchNorm2d(hidden_size)
        self.relu1_2 = nn.ReLU()
        self.bn2_2 = nn.BatchNorm2d(hidden_size)
        self.relu2_2 = nn.ReLU()
    ...
    def forward(self, x_c4, x_c3, x_c2, x_c1):
        ...
        x = self.bn1_4(x)
        x = self.relu1_4(x)
        alphas, angles = self.routing_function1(x)
        # rotated_weight = batch_rotate_multiweight_modified(self.conv2_4.weight.data, alphas, angles)
        rotated_weight = batch_rotate_multiweight(self.conv2_4.weight.data, alphas, angles)
        rotated_weight = rotated_weight.reshape(512, 512, 3, 3)
        x = self.conv2_4(x, rotated_weight)
        x = self.bn2_4(x)
        x = self.relu2_4(x)
        ...
        x = self.bn1_3(x)
        x = self.relu1_3(x)
        alphas, angles = self.routing_function2(x)
        # rotated_weight = batch_rotate_multiweight_modified(self.conv2_3.weight.data, alphas, angles)
        rotated_weight = batch_rotate_multiweight(self.conv2_3.weight.data, alphas, angles)
        rotated_weight = rotated_weight.reshape(512, 512, 3, 3)
        x = self.conv2_3(x, rotated_weight)
        x = self.bn2_3(x)
        x = self.relu2_3(x)
        ...
        x = self.bn1_2(x)
        x = self.relu1_2(x)
        alphas, angles = self.routing_function3(x)
        rotated_weight = batch_rotate_multiweight(self.conv2_2.weight.data, alphas, angles)
        rotated_weight = rotated_weight.reshape(512, 512, 3, 3)
        x = self.conv2_2(x, rotated_weight)
        x = self.bn2_2(x)
        x = self.relu2_2(x)
