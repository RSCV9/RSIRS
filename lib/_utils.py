...
class _LAVTOneSimpleDecode(nn.Module):
    ...
    def forward(self, x, text, l_mask):
        ...
        x = F.interpolate(x, size=input_shape, mode='bilinear', align_corners=True)
        return x
        # return x, {"c1": x_c1, "c2": x_c2, "c3": x_c3, "c4": x_c4}

    @torch.no_grad()
    def infer_with_features(self, x, text, l_mask):
        """推理同时返回四级特征，方便可视化。"""
        input_shape = x.shape[-2:]
        ...
        features = self.backbone(x, l_feats, l_mask)
        x_c1, x_c2, x_c3, x_c4 = features
        logits = self.classifier(x_c4, x_c3, x_c2, x_c1)
        logits = F.interpolate(logits, size=input_shape, mode='bilinear', align_corners=True)
        return logits, {"c1": x_c1, "c2": x_c2, "c3": x_c3, "c4": x_c4}
